...
const TEST_DB = path.join(os.tmpdir(), `review-test-${Date.now()}-${process.pid}.db`);
const TEST_CONFIG_DIR = path.join(os.tmpdir(), `codecortex-review-test-${Date.now()}`);
...
describe("isQuestion heuristic", () => {
  ...
  it("returns true for common question patterns", () => {
    assert.equal(isQuestion("Do you want me to also refactor the tests?"), true);
  ...
  it("returns false for completion-style messages", () => {
    ...
    assert.equal(isQuestion("I've updated the auth module as requested."), false);
  });
});
...
describe("PATCH /api/review/config", () => {
  ...
  it("rejects invalid reviewMode values", async () => {
    const res = await req("PATCH", "/api/review/config", { reviewMode: "invalid" });
    assert.equal(res.status, 400);
  });

  it("persists reviewModel provider change", async () => {
    await req("PATCH", "/api/review/config", {
      reviewModel: { provider: "kimi", apiKey: "test-key", model: "moonshot-v1-8k" },
    });
    const res = await req("GET", "/api/review/config", null);
