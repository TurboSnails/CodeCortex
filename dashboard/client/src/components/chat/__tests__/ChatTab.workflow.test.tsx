...
describe("ChatTab workflow mode", () => {
  ...
  it("stops auto-advance on a quick permission rejection and retry resends the current step", async () => {
    mockStart.mockResolvedValueOnce(makeHandle());
    mockSend.mockResolvedValueOnce({ messageId: "msg-2" });
    mockRespondToPermission.mockResolvedValueOnce({ ok: true });
    ...
    try {
      ...
      fireEvent.click(screen.getByRole("button", { name: /retry/i }));
      await waitFor(() => expect(mockSend).toHaveBeenCalledWith("run-1", "/opsx:explore", []));
    } finally {
      vi.useRealTimers();
    }
  });
